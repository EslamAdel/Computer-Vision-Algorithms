...
def onClick(event):
    """Call back function to run the region growing algorithm

    Args:
        event (MouseClick): the mouse click event
    """
...
def showSegmentedImage(region):
    """Show the result of region growing algorithm

    Args:
        region (np.array): the segmented region
    """
...
def regionGrowing(image, seed, threshold):
    ...
    Args:
        image (np.array): gray image to be
        seed (tuple): starting point
        threshold (int): threshold for joining the region

    Returns:
        np.array: the segmented region
    """
