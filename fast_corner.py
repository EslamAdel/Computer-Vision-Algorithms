...
def weighFunction(image, p):
    ...
    try:
        ...
        subIm = np.array([image[i+3, j-1:j+2], image[i-3, j-1:j+2], image[i-1:i+2, j+3],
                          image[i-1:i+2, j-3], image[i+2, j+2], image[i-2, j+2], image[i-2, j-2], image[i+2, j-2]])
...
def plot_corner_points(image, filtered_coords):
    ...
    plt.set_cmap('gray')
    plt.imshow(image)
    plt.scatter([p[1] for p in filtered_coords], [p[0]
                                                  for p in filtered_coords], marker='o', c='b', sizes=(10,))
    plt.axis('off')
    plt.show()
...
if __name__ == '__main__':
    ...
    corners = fastDetect(hsvImage[..., 2], 120)
